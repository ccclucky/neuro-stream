...
## 1. 核心目标
NeuroStream 作为一个 Agent‑native 的结算协议，其技术栈选型优先考虑 **极致性能 (Monad)**、**安全稳健 (Hardhat/Solidity)** 和 **低延迟索引 (Envio)**，同时利用 **Supabase** 降低后端运维复杂度。
...
*   **Indexer (索引器)**: [Envio (HyperIndex)](https://envio.dev/)
    *   **理由**: 采用 Rust 开发的高性能索引方案，支持 HyperSync 极速同步，确保数据延迟最低。
...
| 决策维度 | 选型 | 排除原因 |
| :--- | :--- | :--- |
| **Indexer** | **Envio** | Ponder 虽然 DX 更好，但其 Node.js 底层可能无法支撑 Monad 的高吞吐量。 |
